...
const PROMPT = `Given the job title "{jobTitle}",
 create 3-5 concise and personal summaries in
  ...
  skills relevant technologies and significant
   contributions in that role. Do not include
    the job title itself in the output. Provide
     only the summaries inside an unordered
     list.`;
...
const RichTextEditor = (props: {
  ...
  const GenerateSummaryFromAI = async () => {
    try {
      ...
      const validJsonArray = responseText.replace("[", "").replace("]", "").replace('"','').replace('"','').replace('{ bullet_points:','');

      setValue(validJsonArray);
      onEditorChange(validJsonArray);
    } catch (error) {
      console.log(error);
      toast({
